...
class PluginOptions
{
	...
	static function Get( $ver, $name, $cbNs = '', $rawExtData = null )
	{
		...
		else
		{
			...
			$data = $dataRaw = Plugin::IsInited() ? get_option( 'seraph_osm_' . $name ) : null;
		...
		if( $ver != $verFrom )
		{
			...
			if( !$rawExtData )
			{
				if( $verFrom && Plugin::IsInited() )
					update_option( 'seraph_osm_' . $name . 'V' . $verFrom, $dataRaw );
	...
	static function Set( $ver, $name, $data, $cbNs = '', $updateCacheOnly = false )
	{
		...
		if( Plugin::IsInited() )
			$hr = update_option( 'seraph_osm_' . $name, @json_encode( $dataWrite ) ) ? Gen::S_OK : Gen::S_FALSE;
		else
	...
	static function Del( $name, $cbNs = '' )
	{
		...
		delete_option( 'seraph_osm_' . $name );
...
class PluginPostOptions
{
	...
	static function Get( $postId, $ver, $name, $cbType = 'Post', $cbNs = '' )
	{
		...
		$data = $dataRaw = get_post_meta( $postId, '_seraph_osm_' . $name, true );
		if( !is_array( $data ) )
		{
			$data = @json_decode( $data, true );
			if( !is_array( $data ) )
		...
		{
			if( $verFrom )
				update_post_meta( $postId, '_seraph_osm_' . $name . 'V' . $verFrom, $dataRaw );
	...
	static function Set( $postId, $ver, $name, $data, $cbType = 'Post', $cbNs = '' )
	{
		...
		return( update_post_meta( $postId, '_seraph_osm_' . $name, @json_encode( $dataWrite ) ) );
	}
	...
	static function Del( $postId, $name, $cbType = 'Post' )
	{
		...
		delete_post_meta( $postId, '_seraph_osm_' . $name );
		return( true );
	}
}
...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '1.4' || $lastCheckPackage !== 'Base' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '1.4' && !isset( $state[ 'changeVerCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '1.4' )
				return( Gen::S_FALSE );
		...
		$urlRemoteCfg = null;
		{
			...
			$args[ 'id' ] = 'wordpress-old-slugs-mgr';
			$args[ 'name' ] = 'Alternative%20Slugs%20Manager';
			$args[ 'v' ] = '1.4';
			$args[ 'pk' ] = 'Base';
			$args[ 'cfg' ] = '';
			$args[ 'loc' ] = Wp::GetLocale();

			$urlRemoteCfg = add_query_arg( $args, 'https://www.s-sols.com/data/products/wordpress/old-slugs-mgr/cfg0001.json.txt' );
		}
...
class Plugin
{
	const BASENAME			= 'seraphinite-old-slugs-mgr/plugin_root.php';
	...
	static function Init()
	{
		...
		if( $isAdminMode )
		{
			add_action( 'wp_ajax_seraph_osm_act', __CLASS__ . '::_on_admin_action_act' );

			add_filter( 'removable_query_args',
				function( $args )
				{
					$args[] = 'seraph_osm_postOpsRes';
					return( $args );
				}
			);
			...
			add_action( 'admin_init',
				function()
				{
					...
					$settingsOp = Wp::SanitizeTextData( (isset($_REQUEST[ 'seraph_osm_settingsOp' ])?$_REQUEST[ 'seraph_osm_settingsOp' ]:null) );
					if( $settingsOp )
					{
						if( $settingsOp == 'reset' )
				...
				function( $res )
				{
					...
					if( $settingsOp )
					{
						if( $settingsOp[ 'op' ] == 'reset' )
						{
							$hr = $settingsOp[ 'hr' ];
							if( $hr == Gen::S_OK )
								echo( Ui::BannerMsg( Ui::MsgSucc, esc_html_x( 'ResetSuccInfo', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), Ui::MsgOptDismissible ) );
							else
								echo( Ui::BannerMsg( Ui::MsgErr, sprintf( esc_html_x( 'ResetErrInfo_%1$s', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), Plugin::GetErrorDescr( $hr ) ), Ui::MsgOptDismissible ) );
						}
					}
				}
			);
		...
		{
			if( $isAdminMode )
				Gen::CallFunc( 'seraph_osm\\OnInitAdminModeNotAccepted' );
		...
		add_action( 'wp_loaded',
			function()
			{
				if( isset( $_REQUEST[ 'seraph_osm_api' ] ) )
					self::_OnApiCall( '', 'seraph_osm_api' );
			}
		, -999999 );

		add_filter( 'plugins_update_check_locales', __CLASS__ . '::_on_check_plugins_updates', 10, 1 );

		add_action( 'admin_post_nopriv_seraph_osm_api', function() { unset( $_REQUEST[ 'action' ] ); self::_OnApiCall( '', 'fn' ); } );
		add_action( 'admin_post_seraph_osm_api', function() { unset( $_REQUEST[ 'action' ] ); self::_OnApiCall( '', 'fn' ); } );

		if( $isAdminMode )
		{
			add_action( 'wp_ajax_seraph_osm_api', function() { unset( $_REQUEST[ 'action' ] ); self::_OnApiCall( 'Admin', 'fn' ); } );
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					Plugin::ActionsListAdd( $actions, 'docs', Ui::Link( esc_html_x( 'PluginDocLink', 'admin.Common', 'seraphinite-old-slugs-mgr' ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlProductDocs' ), true, array( 'noTextIfNoHref' => true ) ), true );
					if( !Gen::IsEmpty( $urlSettings = menu_page_url( 'seraph_osm_settings', false ) ) )
					...
					if( !Gen::IsEmpty( Gen::GetArrField( $rmtCfg, 'Prms.FullProductDownloadPath' ) ) )
						Plugin::ActionsListAdd( $actions, 'order', Ui::Link( esc_html_x( 'OrderInLockedFeatureBtn', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlProductBuy' ), true, array( 'noTextIfNoHref' => true ), array( 'style' => array( 'font-weight' => '900' ) ) ), true );
					...
					if( $actDeactivate )
					{
						...
						if( $q )
						{
							...
							$href = null;
							{
								...
								HtmlNd::SetAttrVal( $ndAct, 'onclick', 'seraph_osm.Ui.PopupShow(\'deactivateQuestionnaire\');return(false);' );
			...
			Gen::CallFunc( 'seraph_osm\\OnInitAdminMode' );
		}

		Gen::CallFunc( 'seraph_osm\\OnInit', array( $isAdminMode ) );

		return( array( 'isAdmin' => $isAdminMode ) );

		esc_html__( 'Seraphinite Alternative Slugs Manager', 'seraphinite-old-slugs-mgr' );
		esc_html__( 'Seraphinite Alternative Slugs Manager', 'seraphinite-old-slugs-mgr' );
		esc_html__( 'Avoid URL redirection errors by managing old and alternative slugs to improve SEO.', 'seraphinite-old-slugs-mgr' );
		esc_html__( 'https://www.s-sols.com', 'seraphinite-old-slugs-mgr' );
		esc_html__( 'Seraphinite Solutions', 'seraphinite-old-slugs-mgr' );
		esc_html__( 'http://wordpress.org/plugins/seraphinite-old-slugs-mgr', 'seraphinite-old-slugs-mgr' );
	}
	...
	static function _OnLoadLoc()
	{
		...
		self::$_locale = Wp::Loc_Load( $subSystemIds, 'seraphinite-old-slugs-mgr', Plugin::GetCurBaseName( false ), 'languages', apply_filters( 'seraph_osm_onLocLoadAddFiles', array(), 'languages' ) );
	}
	...
	static private function _OnApiCall( $m, $fn )
	{
		...
		self::_ApiCall_Make( 'seraph_osm\\On' . $m . 'Api_' . $apiFunc, $_REQUEST );
		exit();
	}

	static function Loc_ScriptLoad( $handle )
	{
		return( Wp::Loc_ScriptLoad( $handle, 'seraphinite-old-slugs-mgr', Plugin::GetCurBaseName( false ), 'languages' ) );
	}
	...
	static function ReloadWithPostOpResEx( $res, $redir = null )
	{
		...
		return( add_query_arg( array( 'seraph_osm_postOpsRes' => rawurlencode( base64_encode( json_encode( $res ) ) ) ), $redir ) );
	}
	...
	static function GetSwitchToExtTitle()
	{
		...
		return( esc_html_x( 'ExtTitle', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ) );
	...
	static function SwitchToExt( $type = Plugin::DisplayContent_Block, $text = null )
	{
		...
		if( !empty( $text ) )
			$res .= $text . ' ';
		$res .= vsprintf( Wp::safe_html_x( 'ExtInfo_%1$s%2$s', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ), Ui::Link( Ui::Tag( 'strong', array( '', '' ) ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlProductFeatures' ), true ) );

		return( Plugin::_GetSwitchToContent( $rmtCfg,

			$type == Plugin::DisplayContent_SmallBlock ? esc_html_x( 'ExtLinkSmallBtn', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ) : esc_html_x( 'ExtLinkBtn', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ),
	...
	static function _GetSwitchToContent( $rmtCfg, $switchBtnName, $mode, $text = null, $isBtmPrimary = true )
	{
		...
		if( $text )
			$res .= ' ';
		$res .= sprintf( Wp::safe_html_x( 'ContentNotInplaceInfo_%1$s', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ), $switchBtnName );

		if( !self::_IsEulaAccepted() )
		{
			if( !empty( $res ) )
				$res .= ' ';
			$res .= vsprintf( esc_html_x( 'LicAcceptInfo_%1$s%2$s%3$s', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ), Gen::ArrFlatten( array( $switchBtnName, Ui::Link( array( '', '' ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlEula' ), true ) ) ) );
		}

		$res .= Ui::TagClose( 'p' );

		$res .= Ui::TagOpen( 'div', array( 'class' => 'ctlSpaceVBefore seraph_osm_switchto' ) );
		{

			$res .= Ui::Button( $switchBtnName, $isBtmPrimary, null, 'seraph_osm_btnok', 'button', array( 'onclick' => 'seraph_osm.PluginAdmin.SwitchTo("' . $mode . '","' . add_query_arg( array( 'action' => 'seraph_osm_act', 'fn' => 'acceptEula' ), get_admin_url( NULL, 'admin-ajax.php' ) ) . '","' . Plugin::RmtCfgFld_GetLoc( PluginRmtCfg::Get(), 'Links.UrlProductInfo' ) . '")', 'class' => 'ctlSpaceAfter ctlVaMiddle' ) );
	...
	static private function _GetRateItUrl( $rmtCfg )
	{
		...
		if( !$url )
			$url = 'https://wordpress.org/support/plugin/seraphinite-old-slugs-mgr/reviews?rate=5#new-post';
		return( $url );
	}

	static function GetRateItTitle()
	{
		return( esc_html_x( 'Title', 'admin.Common_RateIt', 'seraphinite-old-slugs-mgr' ) );
	}
	...
	static function GetRateItContent( $blockId, $type = Plugin::DisplayContent_Block )
	{
		...
		$res .= '<br/><br/>' . Ui::Button( esc_html_x( 'RateSmallBtn', 'admin.Common_RateIt', 'seraphinite-old-slugs-mgr' ), true, null, 'ctlSpaceAfter', 'button', array( 'onclick' => 'window.open(\'' . self::_GetRateItUrl( $rmtCfg ) . '\', \'_blank\')' ) );

		$res .= Ui::Tag( 'span',
			Ui::Tag( 'input', null, array( 'type' => 'button', 'class' => 'button-link', 'style' => array( 'margin-right' => '1em', 'vertical-align' => 'middle' ), 'value' => esc_html( Wp::GetLocString( 'Dismiss' ) ), 'onclick' => 'seraph_osm.PluginAdmin.RateItCont_Set(\'' . $blockId . '\',false,\'' . add_query_arg( array( 'action' => 'seraph_osm_act', 'fn' => '' ), get_admin_url( null, 'admin-ajax.php' ) ) . '\');return false;' ) ),
			array( 'class' => 'actions' )
		);
	...
	static function _GetRateItContent( $type )
	{
		...
		$res = vsprintf( esc_html_x( 'Info_%1$s%2$s%3$s%4$s%5$s', 'admin.Common_RateIt', 'seraphinite-old-slugs-mgr' ), Gen::ArrFlatten( array(
			Ui::Link( '&#9733;&#9733;&#9733;&#9733;&#9733;', self::_GetRateItUrl( $rmtCfg ), true ),
			$type == Plugin::DisplayContent_Str ? '' : ' ' . esc_html_x( 'Info_P2', 'admin.Common_RateIt', 'seraphinite-old-slugs-mgr' ),
	...
	{
		return( sprintf( esc_html_x( 'PluginSubjectTitle_%1$s%2$s', 'admin.Common', 'seraphinite-old-slugs-mgr' ), Plugin::GetPluginString( 'TitleLong' ), $name ) );
	}

	static function GetPluginString( $id )
	{
		$id = 'Plugin' . $id;
		return( esc_html_x( $id, 'admin.Common', 'seraphinite-old-slugs-mgr' ) );

		esc_html_x( 'PluginTitle', 'admin.Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'PluginTitleLong', 'admin.Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'PluginTitleFull', 'admin.Common', 'seraphinite-old-slugs-mgr' );

		esc_html_x( 'PluginDescription', 'admin.Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'PluginDescriptionFull', 'admin.Common', 'seraphinite-old-slugs-mgr' );

		esc_html_x( 'PluginSubjectTitle_%1$s%2$s', 'admin.Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'PluginNameToDetails_%1$s%2$s', 'admin.Common', 'seraphinite-old-slugs-mgr' );

		esc_html_x( 'Start', 'admin.Common', 'seraphinite-old-slugs-mgr' );

		esc_html_x( 'FuncBlocked_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'ExecErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'ExecErrCode_%1$s%2$d%3$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'TmpFileCreateErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'DirWriteErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'FileWriteErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'FileReadErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'FileModifyErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'FileRenameErr_%1$s%2$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'FileDeleteErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'FileMakeExecErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );

		esc_html_x( 'NetDownloadErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'NetMimeErr_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );

		esc_html_x( 'PhpExecMdlNotFound_%1$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'PhpExtDirSuffix_%1$s%2$s', 'admin.Common_Msg', 'seraphinite-old-slugs-mgr' );
	}

	static function GetAboutPluginTitle()
	{
		return( esc_html_x( 'Title', 'admin.Common_About', 'seraphinite-old-slugs-mgr' ) );
	}
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '1.4' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '1.4' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
...

		{
			...
			$res .= '<h3 style="margin:0">' . esc_html_x( 'PluginTitleFullWithSuffix', 'admin.Common', 'seraphinite-old-slugs-mgr' ) . '</h3>';

			$res .= Ui::Tag( 'div', vsprintf( esc_html( esc_html_x( 'Version_%1$s%2$s%3$s', 'admin.Common_About', 'seraphinite-old-slugs-mgr' ) ), Gen::ArrFlatten( array( Ui::Tag( 'strong', array( '', '' ) ), $version ) ) ), array( 'class' => 'pluginVersion', 'style' => array( 'margin-top' => '0.3em' ) ) );

			$res .= Ui::TagClose( 'div' );
		}

		$res .= Ui::Tag( 'p', esc_html_x( 'PluginDescriptionFull', 'admin.Common', 'seraphinite-old-slugs-mgr' ) );

		{
			...
			$resPart .= Ui::Link( esc_html_x( 'PluginDocLink', 'admin.Common', 'seraphinite-old-slugs-mgr' ), $urlAboutPluginDocs, true, $linksPrms );

			$resPart .= Ui::Link( esc_html_x( 'Link3rdPartySoft', 'admin.Common_About', 'seraphinite-old-slugs-mgr' ), $url3rdPartySoft, true, $linksPrms );
...

		{
			...
			if( !empty( $urlAboutPluginSupport ) )
				$resPart .= Ui::Button( esc_html_x( 'LinkSupport', 'admin.Common_About', 'seraphinite-old-slugs-mgr' ), false, null, 'ctlSpaceAfter', 'button', array( 'onclick' => 'window.open( \'' . $urlAboutPluginSupport . '\', \'_blank\' )' ) );
	...
	{
		return( esc_html_x( 'Title', 'admin.Common_AboutVendor', 'seraphinite-old-slugs-mgr' ) );
	}
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '1.4' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
		...
		$res .= Ui::Tag( 'p', esc_html_x( 'Info1', 'admin.Common_AboutVendor', 'seraphinite-old-slugs-mgr' ) );

		$res .= Ui::Tag( 'p', vsprintf( esc_html_x( 'Info2_%1$s%2$s%3$s%4$s%5$s%6$s', 'admin.Common_AboutVendor', 'seraphinite-old-slugs-mgr' ), Gen::ArrFlatten( array(
...

		{
			...
			if( !empty( $urlMorePlugins ) )
				$resPart .= Ui::Button( esc_html_x( 'MorePluginsBtn', 'admin.Common_AboutVendor', 'seraphinite-old-slugs-mgr' ), false, null, 'ctlSpaceAfter', 'button', array( 'onclick' => 'window.open( \'' . $urlMorePlugins . '\', \'_blank\' )' ) );

			if( !empty( $urlMoreInfo ) )
				$resPart .= Ui::Button( esc_html_x( 'MoreInfoBtn', 'admin.Common_AboutVendor', 'seraphinite-old-slugs-mgr' ), false, null, 'ctlSpaceAfter', 'button', array( 'onclick' => 'window.open( \'' . $urlMoreInfo . '\', \'_blank\' )' ) );
	...
	{
		return( esc_html_x( 'SettingsTitle', 'admin.Common_Lic', 'seraphinite-old-slugs-mgr' ) );
	}
	...
	static function GetSettingsLicenseContent()
	{
		...
		$res = '';
		$res .= vsprintf( Wp::safe_html_x( 'ExtLicenseInfo_%1$s%2$s', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ), Ui::Link( Ui::Tag( 'strong', array( '', '' ) ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlProductFeatures' ), true ) );

		return( Plugin::_GetSwitchToContent( $rmtCfg,

			esc_html_x( 'ExtLinkBtn', 'admin.Common_SwitchTo', 'seraphinite-old-slugs-mgr' ),
	...
	{
		return( esc_html_x( 'Title', 'admin.Common_AdvertProducts', 'seraphinite-old-slugs-mgr' ) );
	}
	...
	static function GetAdvertProductsContent( $idBlock )
	{
		...
		$licHtmlContent = Plugin::GetLockedFeatureLicenseContent( Plugin::DisplayContent_Block, '', vsprintf( esc_html_x( 'LockedFeatureInsideInfo_%1$s%2$s%3$s%4$s', 'admin.Common_AdvertProducts', 'seraphinite-old-slugs-mgr' ), Gen::ArrFlatten( array( Ui::Tag( 'strong', array( '', '' ) ), Ui::Tag( 'strong', array( '', '' ) ) ) ) ) );
		...
		$res .= Ui::Tag( 'div',
			...
			array( 'data-oninit' => 'seraph_osm.PluginAdmin.AdvertProductsContentInit(this,"' . esc_url( $urlRequest ) . '","' . Wp::GetLocale() . '","' . esc_html_x( 'ItemMoreInfoBtn', 'admin.Common_AdvertProducts', 'seraphinite-old-slugs-mgr' ) . '",' . self::ADVERTPRODUCTS_COLS_NUM . ',' . self::ADVERTPRODUCTS_IMG_WIDTH . ')' ) );

		$res .= Ui::Tag( 'p', vsprintf( esc_html_x( 'TotalInfo_%1$s%2$s', 'admin.Common_AdvertProducts', 'seraphinite-old-slugs-mgr' ), Ui::Link( Ui::Tag( 'strong', array( '', '' ) ), $urlMorePlugins, true ) ), array( 'style' => array( 'text-align' => 'center' ) ) );
		if( !empty( $urlMorePlugins ) )
			$res .= Ui::Tag( 'p', Ui::Button( esc_html_x( 'MorePluginsBtn', 'admin.Common_AboutVendor', 'seraphinite-old-slugs-mgr' ), false, null, 'ctlSpaceAfter', 'button', array( 'onclick' => 'window.open( \'' . $urlMorePlugins . '\', \'_blank\' )' ) ), array( 'style' => array( 'text-align' => 'center' ) ) );
	...
	static function Sett_SaveBtn( $name, $full = true )
	{
		...
		$urlAct = add_query_arg( array( 'action' => 'seraph_osm_act', 'fn' => '' ), get_admin_url( NULL, 'admin-ajax.php' ) );

		$res .=
			Ui::Button( esc_html( Wp::GetLocString( 'Restore' ) ), false, null, null, 'button', array( 'class' => 'ctlSpaceAfter ctlSpaceVAfter', 'style' => array( 'min-width' => '7em' ), 'onclick' => 'if(confirm("' . esc_html_x( 'RestoreConfirm', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ) . '"))window.location.href="' . add_query_arg( array( 'seraph_osm_settingsOp' => 'reset', '_' => time() , '_wpnonce' => wp_create_nonce( 'reset' ) ), $_SERVER[ 'REQUEST_URI' ] ) . '"' ) ) .
			Ui::Button( esc_html( Wp::GetLocString( 'Import' ) ), false, null, null, 'button', array( 'class' => 'ctlSpaceAfter ctlSpaceVAfter', 'style' => array( 'min-width' => '7em' ), 'onclick' => 'jQuery("#seraph_osm_settings_import_file").trigger("click")' ) ) .
			Ui::Button( esc_html( Wp::GetLocString( 'Export' ) ), false, null, null, 'button', array( 'class' => 'ctlSpaceAfter ctlSpaceVAfter', 'style' => array( 'min-width' => '7em' ), 'onclick' => 'seraph_osm.PluginAdmin.SettExport(this,"' . $urlAct . '")' ) ) .
			Ui::Spinner( false, array( 'class' => 'ctlSpaceAfter', 'style' => array( 'display' => 'none' ) ) ) .
			Ui::InputBox( 'file', 'seraph_osm_settings_import_file', null, array( 'style' => array( 'display' => 'none' ), 'accept' => '.json', 'onchange' => 'seraph_osm.PluginAdmin.SettImport(this,"' . $urlAct . '",this.files.length?this.files[0]:null,"' . wp_create_nonce( 'import' ) . '")' ) ) .
	...
	{
		$res = '';
		$res .= Ui::Button( esc_html_x( 'WizPrevBtn', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), false, null, null, 'button', array( 'class' => 'wizPrev ctlSpaceAfter ctlSpaceVAfter ctlVaMiddle', 'style' => array( 'min-width' => '7em' ), 'disabled' => true, 'onclick' => 'seraph_osm.PluginAdmin.OnSettWizNext(this,false)' ) );
		$res .= Ui::Button( esc_html_x( 'WizNextBtn', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), false, null, null, 'button', array( 'class' => 'wizNext ctlSpaceAfter ctlSpaceVAfter ctlVaMiddle', 'style' => array( 'min-width' => '7em' ), 'disabled' => true, 'onclick' => 'seraph_osm.PluginAdmin.OnSettWizNext(this,true)' ) );
		$res .= Ui::Button( esc_html_x( 'WizFinishBtn', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), true, $name, null, 'submit', array( 'class' => 'wizFinish ctlSpaceAfter ctlSpaceVAfter ctlVaMiddle', 'style' => array( 'min-width' => '7em', 'display' => 'none' ), 'data-oninit' => 'seraph_osm.PluginAdmin.OnSettWizNext(this,false)' ) );
		$res .= Ui::InputBox( 'hidden', '_wpnonce', wp_create_nonce( 'finish' ), null, true );
		return( $res );

		esc_html_x( 'WizStartBtn', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'TitleWiz', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' );
	}

	static function Sett_SaveResultBannerMsg( $hr, $opts = 0, $attrs = null, $errorCtxAlt = null )
	{
		if( $hr == Gen::S_OK || $hr == Gen::S_FALSE )
			return( Ui::BannerMsg( Ui::MsgSucc, esc_html_x( 'SaveSuccInfo', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), $opts, $attrs ) );

		if( Gen::HrSucc( $hr ) )
			return( Ui::BannerMsg( Ui::MsgWarn, sprintf( esc_html_x( 'SaveWarnInfo_%1$s', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), Plugin::GetErrorDescr( $hr, $errorCtxAlt, true ) ), $opts, $attrs ) );

		return( Ui::BannerMsg( Ui::MsgErr, sprintf( esc_html_x( 'SaveErrInfo_%1$s', 'admin.Common_Settings', 'seraphinite-old-slugs-mgr' ), Plugin::GetErrorDescr( $hr, $errorCtxAlt ) ), $opts, $attrs ) );
	}

	static function SettGet( $settRawExt = null )
	{
		return( PluginOptions::Get( PLUGIN_SETT_VER, 'Sett', 'seraph_osm\\', $settRawExt ) );
	}
	...
	{
		return( PluginOptions::Set( PLUGIN_SETT_VER, 'Sett', $data, 'seraph_osm\\', $updateCacheOnly ) );
	}
	...
	{
		return( PluginOptions::Del( 'Sett', 'seraph_osm\\' ) );
	}

	static function DataGet( $dataRawExt = null )
	{
		return( PluginOptions::Get( PLUGIN_DATA_VER, 'Data', 'seraph_osm\\', $dataRawExt ) );
	}

	static function DataSet( $data )
	{
		return( PluginOptions::Set( PLUGIN_DATA_VER, 'Data', $data, 'seraph_osm\\' ) );
	}
	...
	{
		$ver = constant( 'seraph_osm\\PLUGIN_' . strtoupper( $cbType ) . '_SETT_VER' );
		return( PluginPostOptions::Get( $postId, $ver, 'Sett', $cbType, 'seraph_osm\\' ) );
	}

	static function PostSettSet( $postId, $data, $cbType = 'Post' )
	{
		$ver = constant( 'seraph_osm\\PLUGIN_' . strtoupper( $cbType ) . '_SETT_VER' );
		return( PluginPostOptions::Set( $postId, $ver, 'Sett', $data, $cbType, 'seraph_osm\\' ) );
	}

	static function PostDataGet( $postId, $cbType = 'Post' )
	{
		$ver = constant( 'seraph_osm\\PLUGIN_' . strtoupper( $cbType ) . '_DATA_VER' );
		return( PluginPostOptions::Get( $postId, $ver, 'Data', $cbType, 'seraph_osm\\' ) );
	}

	static function PostDataSet( $postId, $data, $cbType = 'Post' )
	{
		$ver = constant( 'seraph_osm\\PLUGIN_' . strtoupper( $cbType ) . '_DATA_VER' );
		return( PluginPostOptions::Set( $postId, $ver, 'Data', $data, $cbType, 'seraph_osm\\' ) );
	}
	...
	{
		return( Plugin::FileUri( 'seraphinite-old-slugs-mgr', null, $siteUrlRelative ) );
	}

	static function GetApiUri2( $funcName = '', $args = array() )
	{
		$res = add_query_arg( array_merge( array( 'action' => 'seraph_osm_api', 'fn' => $funcName ), $args ), get_admin_url( NULL, 'admin-post.php' ) );
		if( empty( $funcName ) )
			$res .= '=';
		return( $res );
	}
	...
	{
		return( isset( $_REQUEST[ 'seraph_osm_api' ] ) );
	}
	...
	static function GetApiUri( $funcName = '', $args = array(), $urlBase = '' )
	{
		...
		$res = add_query_arg( array_merge( array( 'seraph_osm_api' => $funcName ), $args ), $urlBase );
		if( empty( $funcName ) )
			$res .= '=';
		return( $res );
	}
	...
	{
		$res = add_query_arg( array_merge( array( 'action' => 'seraph_osm_api', 'fn' => $funcName ), $args ), Net::Url2Uri( get_admin_url( NULL, 'admin-ajax.php' ) ) );
		if( empty( $funcName ) )
			$res .= '=';
		return( $res );
	}
	...
	{
		$switchToArg = 'seraph_osm_switchto';
	...
	static private function _Sett_Import()
	{
		...
		$data = @json_decode( $data, true );
		if( !$data || (isset($data[ 't' ])?$data[ 't' ]:null) != 'mdl:seraph_osm' )
			return( false );
	...
	static private function _PrevVer_Check()
	{
		...
		if( $plgVerPrev !== null || $plgPkPrev !== null )
		{
			...
			if( Gen::DoesFuncExist( 'seraph_osm\\OnChangeVer' ) )
				Gen::CallFuncArraySafe( 'seraph_osm\\OnChangeVer', array( $plgVerPrev, $plgPkPrev ) );
		}
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '1.4' );
			if( $verTo < $verFrom )
		...
		$url = add_query_arg( array( 'action' => 'seraph_osm_act', 'fn' => '' ), get_admin_url( NULL, 'admin-ajax.php' ) );
...
?>
		...
		<strong>
			<?php echo( esc_html_x( 'PluginTitleFull', 'admin.Common', 'seraphinite-old-slugs-mgr' ) ); ?>
		</strong>
		<p>
			<?php echo( vsprintf( esc_html_x( 'Info_%1$s%2$s', 'admin.Common_ChangeVersion', 'seraphinite-old-slugs-mgr' ), Ui::Link( array( '', '' ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlProductLastChanges' ), true ) ) ); ?>
		</p>

		<input style="margin-right:1em;vertical-align:middle;" type="button" class="button button-primary" value="<?php echo( esc_html_x( 'ConfirmBtn', 'admin.Common_ChangeVersion', 'seraphinite-old-slugs-mgr' ) ); ?>" onclick="seraph_osm.PluginAdmin.ChangeVersion_Confirm('<?php echo( esc_url( $url ) ); ?>');" />
		<span style="display:none;vertical-align:middle;" class="seraph_osm_spinner"></span>

		<?php

		$res = ob_get_clean();

		return( Ui::BannerMsg( Ui::MsgWarn, $res, 0, array( 'id' => 'seraph_osm_ChangeVersion_Msg' ) ) );
	}
	...
	static private function _RateIt_Check()
	{
		...
		<strong><?php echo( esc_html_x( 'PluginTitleFull', 'admin.Common', 'seraphinite-old-slugs-mgr' ) ); ?></strong>
		<p><?php echo( esc_html_x( 'InfoPrefix', 'admin.Common_RateIt', 'seraphinite-old-slugs-mgr' ) ); ?> <?php echo( self::_GetRateItContent( Plugin::DisplayContent_Block ) ); ?></p>

		<input style="margin-right:1em;vertical-align:middle;" type="button" class="button button-primary" value="<?php echo( esc_html_x( 'RateBtn', 'admin.Common_RateIt', 'seraphinite-old-slugs-mgr' ) ); ?>" onclick="window.open('<?php echo( self::_GetRateItUrl( $rmtCfg ) ); ?>','_blank');" />

		<span class="actions">
			<input style="margin-right:1em;vertical-align:middle;" type="button" class="button-link" value="<?php echo( esc_html_x( 'PostponeBtn', 'admin.Common_RateIt', 'seraphinite-old-slugs-mgr' ) ); ?>" onclick="seraph_osm.PluginAdmin.RateIt_Set(true,'<?php echo( esc_url( $url ) ); ?>');" />
			<input style="margin-right:1em;vertical-align:middle;" type="button" class="button-link" value="<?php echo( esc_html( Wp::GetLocString( 'Dismiss' ) ) ); ?>" onclick="seraph_osm.PluginAdmin.RateIt_Set(false,'<?php echo( esc_url( $url ) ); ?>');" />
		</span>
		...
		return( Ui::BannerMsg( Ui::MsgInfo, $res, 0, array( 'id' => 'seraph_osm_RateIt_Message' ) ) );
	}
	...
	static private function _Eula_Check()
	{
		...
		$acceptBtnName = esc_html_x( 'AcceptBtn', 'admin.Common_Eula', 'seraphinite-old-slugs-mgr' );

		$res = '';
		$res .= '<strong>' . esc_html_x( 'PluginTitleFull', 'admin.Common', 'seraphinite-old-slugs-mgr' ) . '</strong>';

		$res .= '<p>' . vsprintf( esc_html_x( 'AcceptInfo_%1$s%2$s%3$s', 'admin.Common_Eula', 'seraphinite-old-slugs-mgr' ), Gen::ArrFlatten( array(
			Ui::Link( array( '', '' ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlEula' ), true ),
			$acceptBtnName
		) ) ) . '</p>';

		$res .= '<input type="button" class="button button-primary" value="' . $acceptBtnName . '" style="margin-right: 0.5em;" onclick="window.location.href=\'' . add_query_arg( array( 'action' => 'seraph_osm_act', 'fn' => 'acceptEula', 'redir' => rawurlencode( $_SERVER[ 'REQUEST_URI' ] ), '_' => time()  ), get_admin_url( NULL, 'admin-ajax.php' ) ) . '\'" />';
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '1.4' );
	...
	{
		if( !is_array( $id ) )
			return( 'seraph_osm_' . ( $prefix ? ( $prefix . '_' ) : '' ) . $id );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '1.4' );
			if( $id == 'Gen' )
				Plugin::Loc_ScriptLoad( $scrHndId );
			wp_enqueue_script( $scrHndId );

			if( $id == 'Gen' )
				Wp::AddInlineScript( $scrHndId, 'seraph_osm.Plugin._int.urlRoot="' . wp_slash( Plugin::FileUri( '', __DIR__ ) ) . '";seraph_osm.Plugin._int.urlAdminApi="' . wp_slash( Plugin::GetAdminApiUri() ) . '";seraph_osm.Wp.Loc._int.lang="' . Lang::GetLangFromLocale( Wp::GetLocale() ) . '";' );
	...
	static function GetErrorDescr( $hr, $ctxAlt = null, $tryFindWarnAsError = false )
	{
		...
		foreach( $keysToFind as $keyToFind )
		{
			...
			$errDescr = sprintf( esc_html_x( $id, $keyToFind[ 'ctx' ], 'seraphinite-old-slugs-mgr' ), $hr );
			if( $errDescr != $id )
				return( $errDescr );
		}
		...
		esc_html_x( 'Err_80004001',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_80004005',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_80004021',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_80070005',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_80070057',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_80070490',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_80070570',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_800705B4',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_800401F7',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Err_8007078B',		'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
		esc_html_x( 'Def_%08X', 'admin.ErrDescr_Common', 'seraphinite-old-slugs-mgr' );
	}
	...
	{
		global $seraph_osm_g_plgPrintScriptsStyles;

		if( $seraph_osm_g_plgPrintScriptsStyles )
			return;

		$seraph_osm_g_plgPrintScriptsStyles = true;
	...
	{
		$opsRes = Wp::SanitizeTextData( (isset($_REQUEST[ 'seraph_osm_postOpsRes' ])?$_REQUEST[ 'seraph_osm_postOpsRes' ]:null) );
		if( !empty( $opsRes ) )
...

		{
			if( version_compare( PHP_VERSION, '5.4' ) < 0 )
			{
				...
				echo( Ui::BannerMsg( Ui::MsgErr,
					Ui::Tag( 'strong', esc_html_x( 'PluginTitleFull', 'admin.Common', 'seraphinite-old-slugs-mgr' ) ) .
					Ui::Tag( 'p', vsprintf( Wp::safe_html_x( 'PhpMinVerErr_%1$s%2$s%3$s%4$s', 'admin.Common', 'seraphinite-old-slugs-mgr' ), Gen::ArrFlatten( array(
						PHP_VERSION, '5.4',
			...
			if( version_compare( $wp_version, '4.5' ) < 0 )
			{
				...
				echo( Ui::BannerMsg( Ui::MsgErr,
					Ui::Tag( 'strong', esc_html_x( 'PluginTitleFull', 'admin.Common', 'seraphinite-old-slugs-mgr' ) ) .
					Ui::Tag( 'p', vsprintf( Wp::safe_html_x( 'WpMinVerErr_%1$s%2$s%3$s%4$s', 'admin.Common', 'seraphinite-old-slugs-mgr' ), Gen::ArrFlatten( array(
						$wp_version, '4.5',
						Ui::Link( array( '', '' ), Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Links.UrlProductReqs' ), true )
					) ) ) )
					, 0 ) );
		...
		if( !empty( $opsRes ) )
			do_action( 'seraph_osm_postOpsRes', $opsRes );
	}
	...
	static function _OnPopup_QuestionnaireDeactivate( $id, $prms )
	{
		...
		Ui::PostBoxes_Popup( $id, Plugin::RmtCfgFld_GetLocEx( $rmtCfgFldCtx, $qParams, 'Title' ),
			function( $callbacks_args, $box )
			{
				...
				{
					$groupName = 'seraph_osm_questionId';
...
?>
				...
				<script>
					(function()
					{
						...
						box.find( ".actCancel" ).on( "click",
							function()
							{
								seraph_osm.Ui.BannerMsgClose( box );
							}
						);
						...
						{
							box.find( ".seraph_osm_spinner" ).hide();
							seraph_osm.Ui.BannerMsgClose( box );
							//
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							box.find( ".seraph_osm_spinner" ).show();
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "1.4" ) );
	...
	static function OnActivate()
	{
		...
		Gen::CallFunc( 'seraph_osm\\OnActivate' );
	}

	static function OnDeactivate()
	{

		Gen::CallFunc( 'seraph_osm\\OnDeactivate' );
	}
